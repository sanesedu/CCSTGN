...
from sklearn.metrics import det_curve, roc_auc_score, f1_score, confusion_matrix, average_precision_score, ConfusionMatrixDisplay, DetCurveDisplay, roc_curve

from baselines.mlp import MLP
from baselines.mlr import MLR
from model.ccstgn import CCSTGN
from data_processing.flow_dataset import FlowDataset
from visualization.metrics import comparison_plot, evolution_plots, det_plot
...
def test(device, name, model_name, model, dataloader, loss_criterion, n_classes, lambda_, classifier_head=None, thr=None, run=None):
    ...
    if thr is not None:
        ...
        for t in thr:
            ...
            predictions = [1 if probs_positive[k] >= t else 0 for k in range(len(probs_positive))]

            f1_aux = f1_score(ground_truths, predictions) * 100.
            conf_mat_aux = confusion_matrix(ground_truths, predictions, normalize='true')

            # since the matrix is normalized by true label
            # these computations are correct because they operate over the rows
            tn, fp, fn, tp = conf_mat_aux.ravel()

            fpr_aux = (fp / (fp + tn)) * 100.
            fnr_aux = (fn / (fn + tp)) * 100.
            
            fpr.append(fpr_aux)
            fnr.append(fnr_aux)
            f1.append(f1_aux)
            conf_mat.append(conf_mat_aux.tolist())
        
        with open(f'{RESULTS_PATH}/det_curve_{name}_{run}.pkl', 'wb') as f:
            pickle.dump([fprs, fnrs, thrs], f)
        
        idxs = []
        det_plot([fprs], [fnrs], [idxs], [thr], path=f'{FIGURES_PATH}/det_curve_{run}', names=[name])

    else:
       
        fpr_select = [0.01, 0.1, 1] # percentage
        fpr = []
        fnr = []
        thr = []
        idxs = []
        ...
        for fs in fpr_select:
            idx = [j for j,v in enumerate(fprs) if (v*100) <= fs]
            ...
            if len(idx) == 0:
                ...
                idx = idx[0]
                fpr.append(fprs[idx]*100)
                fnr.append(fnrs[idx]*100)
                thr.append(thrs[idx])
                idxs.append(idx)
                predictions = [1 if probs_positive[k] >= thrs[idx] else 0 for k in range(len(probs_positive))]

                f1.append(f1_score(ground_truths, predictions) * 100.)
                conf_mat.append(confusion_matrix(ground_truths, predictions, normalize='true').tolist())
                
        with open(f'{RESULTS_PATH}/det_curve_{name}.pkl', 'wb') as f:
            pickle.dump([fprs, fnrs, thrs, idxs], f)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--val_thr', nargs='+', default=[], help='Threshold value for testing obtained in validation')
    ...
    if len(args.val_thr) > 0:
        val_thr = list(map(float, args.val_thr))
    ...
    for r in range(n):
        ...
        for ep in range(n_epochs):
            ...
            if args.experiment == 'tuning':
                ...
                loss, auc, f1, fpr, fnr, thr, conf_mat = test(device, model_name, args.model, model, val_loader, loss_criterion, n_classes, lambda_, classifier_head=classifier_head, run=r)

                val_runtime = (time() - start_time)/60.
                logger.info(f'Run {r} - Epoch {ep} - Validation -> time: {val_runtime:.2f} min, loss: {loss}, AUC: {auc}, F1: {f1}, FPR: {fpr}, FNR: {fnr}, thr: {thr}, Confusion matrix: {conf_mat}')
        ...
        if args.experiment == 'tuning':
            ...
            loss, auc, f1, fpr, fnr, thr, conf_mat = test(device, model_name, args.model, model, test_loader, loss_criterion, n_classes, lambda_, classifier_head=classifier_head, thr=val_thr, run=r)

            test_runtime = (time() - start_time)/60.

            logger.info(f'Run {r} - Testing -> time: {test_runtime:.2f} min, loss: {loss}, AUC: {auc}, F1: {f1}, FPR: {fpr}, FNR: {fnr}, thr: {thr}, Confusion matrix: {conf_mat}')
            ...
            logger.info(f'Run {r} - Validation -> time: {test_runtime:.2f} min, loss: {loss}, AUC: {auc}, F1: {f1}, FPR: {fpr}, FNR: {fnr}, thr: {thr}, Confusion matrix: {conf_mat}')
